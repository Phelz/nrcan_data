...
class DataOrganizer:
    ...
    colors: tuple = ("#F8EDD8", "#EEC069", "#7D4E00")
    ...
    def sort_data_availability(self, freq):
        ...
        ind_times_norm = [data_file.time_arr_norm for data_file in data_files]
        ...
        return time_list, time_keeper, time_diff_there, time_diff_notthere, odd_days, ind_times_norm

    def plot_data_availability(self, freq, time_list, time_keeper, time_diff_there, time_diff_notthere, odd_days,  ind_times_norm, ax, legend=False,
                                show_ind_days=True, minor_label_size=15):
        ...
        ax.xaxis.set_tick_params(which="minor", labelsize=minor_label_size, length=7, labelcolor="gray", rotation=0, top=False, direction='out' )
        ...
        for ind_t, ind_t_norm in zip(time_list, ind_times_norm):
            ax.hlines(  y=ind_t_norm, xmin=len(ind_t_norm)*[ind_t - timedelta(hours=6)], xmax=len(ind_t_norm)*[ind_t + timedelta(hours=6)], color=self.colors[2], 
                        zorder=14 , lw=1, alpha=0.5 )

        ax.broken_barh([ (start, diff) for start, diff in zip(time_keeper[::2], time_diff_there) ], (0, 1), facecolors=self.colors[1], zorder=9)
        ax.broken_barh([ (start - timedelta(hours=6), timedelta(hours=12)) for start in odd_days ], (0, 1), facecolors=self.colors[1], zorder=9)
        ...
        ax.set_xlim(time_list[0] - timedelta(days=1), time_list[-1])
        ...
        if legend:
            legend_elements = [ Patch(facecolor=color, edgecolor='k', label=label) for color, label in zip(self.colors, ["Not Available", "Available", "Within Day"]) ]
            ax.legend(handles=legend_elements,  prop={'size': 12},  bbox_to_anchor=(1, 2.1 ), fancybox=True, shadow=True, ncol=2)
        
    
    def show_data_availability(self, show_ind_days=True, minor_label_size=15):
        ...
        for i, (freq, ax) in enumerate( zip(self.frequencies, axs) ):
            ...
            time_list, time_keeper, time_diff_there, time_diff_notthere, odd_days, ind_times_norm = self.sort_data_availability( freq )

            self.plot_data_availability(freq, time_list, time_keeper, time_diff_there, time_diff_notthere, odd_days, ind_times_norm, ax, legend=True if i==0 else False,
                                        show_ind_days=show_ind_days, minor_label_size=minor_label_size)

        fig.suptitle("Data Availability %s" % (self.year), va="top", x=0.5, y=0.97)
